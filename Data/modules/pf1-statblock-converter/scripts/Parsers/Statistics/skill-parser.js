...
export class SkillParser extends ParserBase {
    ...
    async parse(value, line) {
        ...
        try {
            ...
            for (let i=0; i<currentItems.length; i++) {
                ...
                currentItems[i].system.changes?.map(async change => {
                    let skillKey = "";
                    if(/skill\.(...)\.subSkills/i.test(change.target)) { // We are a subskill, and custom
                        skillKey = change.target.match(/skill\....\.subSkills\.(.*)/i)[1];
                    }
                    else if(/skill\./i.test(change.target)) { // We are a normal skill
                        skillKey = change.target.match(/skill\.(.*)/i)[1];
                    }

                    if(skillKey !== "") {
                        let processedFormula = await Roll.fromTerms(Roll.parse(change.formula, sbcData.characterData.actorData.getRollData())).evaluate();
                        processedFormula = processedFormula?.total ?? +change.formula;// change.value;

                        console.log(`Skill: ${skillKey} | Change: ${change.formula} | Processed: ${processedFormula}`);

                        if(skillChanges[skillKey] !== null && skillChanges[skillKey] !== undefined) {
                            skillChanges[skillKey] += processedFormula
                        }
                        else skillChanges[skillKey] = processedFormula;
                    }
                });
            }
            ...
            for (let i=0; i<skills.length; i++) {
                ...
                // Only proceed if it's not a racial modifier
                if (rawSkill.search(/racial (?:modifiers|modifier|bonus)/i) === -1) {
                // Check, if the rawSkill contains "racial modifiers"
                // And skip to the end of the array as we do not need these
                // if (rawSkill.search(/racial (?:modifiers|modifier|bonus)/i) !== -1) {
                ...
                // }
                    ...
                    try {
                        ...
                        if (skillKey !== "skill") {
                            ...
                            if(racialModifiers[skillName.toLowerCase()] && (racialModifiers[skillName.toLowerCase()].context).length === 0) {
                                skillRanks -= racialModifiers[skillName.toLowerCase()].number;
                            }
                            if (skillChanges[skillKey] !== null && skillChanges[skillKey] !== undefined) {
                            ...
                            let defaultAbilityMod = sbcData.characterData.actorData.system.abilities["int"].mod
    ...
    async reparseSkill(skill, maneuverability = null) {
        ...
        const maneuverabilityAdjust = CONFIG["PF1"].flyManeuverabilityValues[maneuverability] || 0;
